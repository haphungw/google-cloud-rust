...
pub async fn sample(project_id: &str, dataset_id: &str) -> anyhow::Result<()> {
    ...
    let dest_table = TableReference::new()
        ...
        .set_table_id("us_states_schema_evolve");

    // Sample data file hosted on Google Cloud Storage.
    // It has columns: name, post_abbr
    let source_uri = "gs://cloud-samples-data/bigquery/us-states/us-states.csv";

    // Imagine the table already has `name` (STRING).
    // Now we append data while defining a new schema that has `name` and `post_abbr`.
    ...
    let job = Job::new().set_configuration(
        JobConfiguration::new().set_load(
            JobConfigurationLoad::new()
                ...
                // Use schema_update_options to allow adding fields
                .set_schema_update_options(vec!["ALLOW_FIELD_ADDITION".to_string()])
                .set_schema(schema),
        ),
    );
    ...
    println!("Created schema update load job: {}", job_ref.job_id);
    ...
    println!("Data loaded successfully with schema update.");
    Ok(())
}
