...
pub async fn run_samples() -> anyhow::Result<()> {
    ...
    let pending: Vec<Pin<Box<dyn Future<Output = anyhow::Result<()>>>>> = vec![
        Box::pin(create_job::sample(&project_id)),
        Box::pin(get_job::sample(&project_id, &test_job_id)),
        Box::pin(list_jobs::sample(&project_id)),
        Box::pin(cancel_job::sample(&project_id)),
    ];
